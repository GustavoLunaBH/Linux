...
# Versão: 2.3 - Com menu de replicação e sincronização
...
check_replication_status() {
    ...
    samba-tool domain listdcs 2>/dev/null || echo "  Não foi possível listar os DCs"
    echo ""
    
    echo -e "${BLUE}=== Informações de Replicação ===${NC}"
    samba-tool drs showrepl 2>/dev/null || echo "  Não foi possível obter informações de replicação"
    echo ""
    
    echo -e "${BLUE}=== Conexões de Replicação ===${NC}"
    samba-tool drs replicate --help 2>/dev/null | head -5 || echo "  Comando não disponível"
    echo ""
    
    echo -e "${BLUE}=== Teste de Conectividade entre DCs ===${NC}"
    if [ -n "$PRIMARY_DC_IP" ]; then
        echo -e "  Testando primário (${PRIMARY_DC_IP}):"
        if ping -c 2 ${PRIMARY_DC_IP} &> /dev/null; then
            success "Primário acessível"
        else
            warning "Primário não acessível"
        fi
    fi
    echo ""
...
force_replication() {
    ...
    # Obter lista de DCs
    local dcs=$(samba-tool domain listdcs 2>/dev/null)
    
    if [ -z "$dcs" ]; then
        error "Não foi possível obter a lista de DCs"
    fi
    ...
    # Forçar replicação para cada DC
    for dc in $dcs; do
        echo -e "${BLUE}Replicando para ${dc}...${NC}"
        
        # Tentar diferentes métodos de replicação
        if samba-tool drs replicate ${dc} ${dc} ${DOMAIN} 2>/dev/null; then
            success "Replicação para ${dc} bem-sucedida"
        elif samba-tool drs replicate ${dc} ${dc} 2>/dev/null; then
            success "Replicação para ${dc} bem-sucedida (método alternativo)"
        else
            warning "Falha na replicação para ${dc}"
        fi
        
        # Forçar sincronização de DNS
        if samba-tool dns replicate ${dc} ${dc} ${DOMAIN} 2>/dev/null; then
            success "DNS replicado para ${dc}"
        fi
...
replicate_from_specific_dc() {
    ...
    read -p "> " TARGET_DC
    [ -z "$TARGET_DC" ] && TARGET_DC="$(hostname)"
    ...
    samba-tool drs replicate ${TARGET_DC} ${SOURCE_DC} ${DOMAIN} 2>/dev/null
        ...
        for dc2 in $dcs; do
            if [ "$dc1" != "$dc2" ]; then
                ...
                if [ "$users1" != "$users2" ]; then
                    warning " ⚠️  Diferença de ${users2} usuários detectada!"
                else
                ...
                if [ "$comps1" != "$comps2" ]; then
                    warning " ⚠️  Diferença de ${comps2} computadores detectada!"
                else
...
check_dns_on_dcs() {
    ...
    for dc in $dcs; do
        ...
        # Verificar se o DC está registrado
        echo -e "${YELLOW}Registro do DC:${NC}"
        samba-tool dns query ${dc} ${DOMAIN,,} $(hostname | cut -d. -f1) A 2>/dev/null | grep "A:" || echo "  Registro não encontrado"
...
setup_auto_replication() {
    ...
    log "Configurando script de replicação automática..."
    ...
    # Criar script de replicação
    cat > /usr/local/bin/force-replication.sh << 'EOF'
#!/bin/bash
# Script de replicação automática AD
LOG="/var/log/ad_replication.log"

echo "$(date) - Iniciando replicação automática" >> $LOG

# Listar DCs
DCS=$(samba-tool domain listdcs 2>/dev/null)

if [ -z "$DCS" ]; then
    echo "$(date) - ERRO: Não foi possível listar DCs" >> $LOG
    exit 1
fi

# Forçar replicação
for dc in $DCS; do
    echo "$(date) - Replicando para ${dc}" >> $LOG
    samba-tool drs replicate ${dc} ${dc} RNV.INTRA 2>/dev/null
    
    if [ $? -eq 0 ]; then
        echo "$(date) - Replicação para ${dc} bem-sucedida" >> $LOG
    else
        echo "$(date) - Falha na replicação para ${dc}" >> $LOG
    fi
done

echo "$(date) - Replicação automática concluída" >> $LOG
echo "----------------------------------------" >> $LOG
EOF
...
EOF
    ...
    success "Script de replicação automática configurado!"
    echo ""
    echo -e "${BLUE}Arquivos criados:${NC}"
    echo -e "  /usr/local/bin/force-replication.sh - Script de replicação"
    echo -e "  /etc/cron.d/ad_replication - Agendamento (a cada 5 minutos)"
    echo -e "  /var/log/ad_replication.log - Log de replicação"
    echo ""
...
REPLICAÇÃO
───────────────────────────────────────────────────────────────────
Verificar status: samba-tool drs showrepl
