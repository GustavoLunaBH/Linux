...
# Versão: 2.2 - Com detecção automática de rede
...
detect_interface() {
    # Detecta a primeira interface de rede ativa (excluindo loopback)
    local interfaces=$(ip -o link show | grep -v "lo:" | grep -v "@" | grep "state UP" | awk -F': ' '{print $2}' | head -1)
    if [ -z "$interfaces" ]; then
        # Se nenhuma interface UP, pega a primeira disponível
...
detect_netplan_file() {
    # Verificar qual arquivo de configuração existe
    if [ -f "/etc/netplan/50-cloud-init.yaml" ]; then
        NETPLAN_FILE="/etc/netplan/50-cloud-init.yaml"
        info "Arquivo de rede detectado: 50-cloud-init.yaml"
    elif [ -f "/etc/netplan/01-netcfg.yaml" ]; then
        NETPLAN_FILE="/etc/netplan/01-netcfg.yaml"
        info "Arquivo de rede detectado: 01-netcfg.yaml"
    elif [ -f "/etc/netplan/00-installer-config.yaml" ]; then
        NETPLAN_FILE="/etc/netplan/00-installer-config.yaml"
        info "Arquivo de rede detectado: 00-installer-config.yaml"
    else
        # Verificar se existe qualquer arquivo .yaml em /etc/netplan/
        local existing_file=$(ls /etc/netplan/*.yaml 2>/dev/null | head -1)
        if [ -n "$existing_file" ]; then
            NETPLAN_FILE="$existing_file"
            info "Arquivo de rede detectado: $(basename $existing_file)"
        else
            # Criar arquivo padrão
            NETPLAN_FILE="/etc/netplan/01-netcfg.yaml"
            info "Nenhum arquivo de rede encontrado. Será criado: 01-netcfg.yaml"
        fi
    fi
}
...
EOF
    ...
    # Remover outros arquivos de rede para evitar conflitos
    log "Removendo arquivos de rede conflitantes..."
...
apply_and_restart_network() {
    ...
    # Verificar se o IP foi aplicado
...
test_connectivity() {
    ...
    echo -e "${BLUE}5. Testando interface:${NC}"
    if ip addr show ${INTERFACE} | grep -q "UP"; then
        success "Interface ${INTERFACE} está UP"
    else
        warning "Interface ${INTERFACE} está DOWN"
    fi
    echo ""
