...
configure_station_ips() {
    ...
    case "$OS_OPT" in
        1)
            # ================================
            # LINUX — NETPLAN INTELIGENTE (MELHORADO)
            # ================================
            ...
            # ② Detectar arquivos Netplan com a função melhorada
            log "Buscando arquivos Netplan em /etc/netplan/..."
            
            # Usa a função detect_netplan_file
            ...
            if [ ${#NETPLAN_FILES[@]} -eq 0 ]; then
                ...
                # Múltiplos arquivos — mostra lista com prioridade
                echo
            ...
            if [ -f "$NETPLAN_PATH" ]; then
                ...
                if grep -q "^[[:space:]]*${STA_IFACE}:" "$NETPLAN_PATH"; then
                    ...
                    # Remove configuração DHCP da interface
                    ...
                    # Insere nova configuração após o nome da interface
                    ...
                    # Interface não existe — adiciona ao final
                    info "Adicionando nova interface $STA_IFACE ao arquivo existente"
                    
                    # Encontra o final da seção ethernets e adiciona
                    awk -v iface="$STA_IFACE" \
                        -v ip="$STA_IP" \
                        -v mask="$STA_MASK" \
                        -v gw="$STA_GATEWAY" \
