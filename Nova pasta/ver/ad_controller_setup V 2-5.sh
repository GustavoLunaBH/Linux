...
# Versão: 2.5 - Com replicação universal e correções
...
check_replication_status() {
    ...
    # Usar método que funciona em todas as versões
    samba-tool domain info 127.0.0.1 2>/dev/null | grep -E "DC name|Domain" | sed 's/^/  /'
    echo ""
    
    echo -e "${BLUE}=== Todos os DCs via DNS ===${NC}"
    ...
    for dc in $(nslookup -type=SRV _ldap._tcp.${DOMAIN,,} 2>/dev/null | grep "svr hostname" | awk '{print $NF}' | sed 's/\.$//'); do
        if [ -n "$dc" ]; then
            echo -n "  Testando ${dc}: "
...
force_replication() {
    ...
    if [ -z "$dcs" ]; then
        dcs=$(samba-tool domain info 127.0.0.1 2>/dev/null | grep "DC name" | awk '{print $NF}')
    fi
...
setup_auto_replication() {
    ...
    # Criar script de replicação universal
