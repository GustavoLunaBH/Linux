...
# Versão: 2.4 - Com menu reorganizado e replicação melhorada
...
check_replication_status() {
    ...
    if [ -f "/var/log/ad_replication.log" ]; then
        tail -10 /var/log/ad_replication.log
    else
    ...
    for dc in $(samba-tool domain listdcs 2>/dev/null); do
        echo -n "  Testando ${dc}: "
...
force_replication() {
    ...
    for dc in $dcs; do
        dc_clean=$(echo $dc | tr -d '[:space:]' | tr '[:upper:]' '[:lower:]')
...
setup_auto_replication() {
    ...
    # Criar script de replicação corrigido
...
log_msg "DC Atual: ${CURRENT_DC}"

DCS=$(samba-tool domain listdcs 2>/dev/null | grep -v "WARNING" | tr '[:upper:]' '[:lower:]')

if [ -z "$DCS" ]; then
    log_msg "ERRO: Não foi possível listar os DCs"
    exit 1
fi

log_msg "DCs encontrados: $DCS"
...
#!/bin/bash
echo "═══════════════════════════════════════════════════════════════════"
echo "  VERIFICAÇÃO DE REPLICAÇÃO AD"
echo "═══════════════════════════════════════════════════════════════════"
echo ""
echo "📋 DCs no Domínio:"
samba-tool domain listdcs 2>/dev/null
echo ""
...
if [ -f "/var/log/ad_replication.log" ]; then
    tail -20 /var/log/ad_replication.log
fi
echo ""
EOF
...
if [ "$INSTALLATION_TYPE" == "primary" ]; then
    ...
    # Se chegou aqui, é porque voltou do menu de rede ou replicação
    show_menu
    get_installation_type
fi
