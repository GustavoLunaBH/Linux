...
# Versão: 3.6 - Com replicação corrigida e DNS automático
...
fix_dns_secondary() {
    ...
    echo -e "${BLUE}6. Reiniciando o Samba para aplicar alterações...${NC}"
    systemctl restart samba-ad-dc
    sleep 5
    
    echo -e "${BLUE}7. Testando resolução DNS...${NC}"
    echo "  Testando resolução do domínio:"
    if nslookup ${DOMAIN_LOWER} 127.0.0.1 2>/dev/null; then
        success "Domínio resolvido: ${DOMAIN_LOWER}"
    else
        warning "Resolução do domínio falhou, mas pode estar funcionando"
    fi
    echo ""
    echo "  Testando resolução do servidor:"
    if nslookup ${HOST}.${DOMAIN_LOWER} 127.0.0.1 2>/dev/null; then
        success "Servidor resolvido: ${HOST}.${DOMAIN_LOWER}"
    else
        warning "Resolução do servidor falhou, mas pode estar funcionando"
    fi
    echo ""
...
# ============================================
# MENU DE REPLICAÇÃO AD - CORRIGIDO
# ============================================
...
replication_menu() {
    ...
    echo -e "     ${CYAN}➜${NC} Puxa alterações do ADServer01 (execute no ADServer02)"
    echo ""
    echo -e "${GREEN} 4)${NC} ${BOLD}REPLICAR DO SECUNDÁRIO PARA O PRIMÁRIO${NC}"
    echo -e "     ${CYAN}➜${NC} Puxa alterações do ADServer02 (execute no ADServer01)"
    echo ""
...
force_replication_manual() {
    ...
    echo -e "${BLUE}2. Replicando de ${other_dc} para ${current_dc}...${NC}"
...
replicate_primary_to_secondary() {
    ...
    if ping -c 3 ${SOURCE_DC} &> /dev/null; then
        success "Primário acessível"
    else
    ...
    echo -e "${BLUE}3. Replicando do primário...${NC}"
    # Tentar replicação normal
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            warning "Falha na replicação forçada. Tentando método alternativo..."
            samba-tool drs replicate ${CURRENT_DC} ${SOURCE_DC} ${DOMAIN} 2>/dev/null
            if [ $? -eq 0 ]; then
                success "Replicação OK (método alternativo)"
            else
                warning "Falha em todos os métodos de replicação"
            fi
        fi
    fi
    
    echo -e "${BLUE}4. Forçando sync-all...${NC}"
    samba-tool drs replicate ${CURRENT_DC} ${CURRENT_DC} ${DOMAIN} --sync-all 2>/dev/null
    if [ $? -eq 0 ]; then
        success "Sync-all OK"
    else
        warning "Sync-all falhou"
    fi
    
    echo -e "${BLUE}5. Verificando status da replicação...${NC}"
    samba-tool drs showrepl 2>/dev/null | head -10
    
    echo -e "${BLUE}6. Verificando usuários...${NC}"
    echo "  Total de usuários: $(samba-tool user list 2>/dev/null | wc -l)"
    
    echo ""
    echo -e "${GREEN}✅ Replicação primário → secundário concluída!${NC}"
    echo -e "${YELLOW}📌 Verifique se os usuários do ADServer01 apareceram no ADServer02${NC}"
...
replicate_secondary_to_primary() {
    ...
    echo -e "${BLUE}3. Replicando do secundário...${NC}"
    samba-tool drs replicate ${TARGET_DC} ${SOURCE_DC} ${DOMAIN} 2>/dev/null
    if [ $? -eq 0 ]; then
        success "Replicação OK"
    else
    ...
    echo -e "${YELLOW}📌 Verifique se os usuários do ADServer02 apareceram no ADServer01${NC}"
...
check_replication_status() {
    ...
    echo "  Total: $(samba-tool user list 2>/dev/null | wc -l)"
    echo "  Últimos 5:"
    samba-tool user list 2>/dev/null | tail -5 | sed 's/^/    /'
    echo ""
